...
## 왜 만들었나 — 자기조건화가 만드는 하나의 답

LLM은 자기가 방금 쓴 토큰을 다시 입력으로 받아 다음 토큰을 만든다. 이 자기조건화
(self-conditioning) 때문에 **처음 잡은 방향이 뒤로 갈수록 강화된다.** 첫 문단에서 "이건
캐시 문제"라고 쓰면 이후 추론은 그 전제를 의심하는 대신 정당화하는 쪽으로 흐른다.
어텐션이 문맥 전체를 볼 수 있다는 것과, 실제로 넓게 보고 있다는 것은 다른 얘기다.

그래서 겉보기에는 자신 있고 매끄러운 답이 하나 나온다. 문제는 그 답이 **여러 후보 중
비교를 거쳐 고른 것이 아니라, 처음 미끄러진 방향을 끝까지 밀어붙인 결과**라는 데 있다.
반복 땜질, 함수 한 줄에 갇힌 오진, 근거 없이 그럴듯한 문장으로 채우는 습관, "다 됐다"는
가짜 완료가 전부 여기서 나온다. 모델이 게을러서가 아니라 구조가 그렇게 되어 있다.

**이 팩은 Claude를 제약해서 창의성을 줄이려는 것이 아니다. 정반대다.**
한 갈래로 수렴하기 전에 멈춰 세우고, 관찰과 해석을 분리하고, 후보를 복수로 놓고,
증거로 판별하게 만든다. 좁아지려는 힘에 맞서 **다시 넓게 보게 하는 장치**다.

| 자기조건화가 하는 일 | 가드팩이 되돌리는 방식 |
|---|---|
| 첫 가설을 뒤에서 계속 정당화 | 관찰·해석·추측을 분리하고 `미확인`을 허용한다 |
| 함수 한 줄에 시야가 갇힘 | 원인과 전파를 가르는 **유관 시스템 경계**까지 넓힌다 |
| 그럴듯한 문장이 증거를 대신함 | 주장마다 실행 증거를 요구하고 미검증 범위를 밝히게 한다 |
| 같은 방향으로 계속 땜질 | 새 판별 정보가 없으면 자동 수리를 멈추게 한다 |
| 기억으로 사실을 채움 | 바뀔 수 있는 사실은 공식 원문에서 확인하게 한다 |

같은 결론을 여러 에이전트가 반복해도 증거는 하나다. 합의 수는 증거가 아니다 —
이 원칙이 팩 전체를 관통한다.
